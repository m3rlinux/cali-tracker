...
**MINOR 4.2.0:** Impostazioni (`cali_settings`): segnali timer beep HTMLAudio o voce (`speechSynthesis`), volume 50–100%.
...
- Selettore lingua: `IT | EN` nell'header (`setLang()` / `toggleLang()`), scelta in localStorage `cali_lang`, helper `tVariant()` / `getGroupLabel()`
...
--p3: #df60f0  (magenta)
--p4: #f09138  (orange)
--combo: #c8f060 (lime, pool extra Combo)
```
...
- Overlay volume/suoneria Android: `nudgeViewport()` (resize/focus/visualViewport) invalida il layer compositor che lascia la banda bianca; `html::before` fisso `#0c0c0c`
